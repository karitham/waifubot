...
func Give(ctx context.Context, store Store, from, to corde.Snowflake, charID int64) (Character, error) {
	...
	given, err := store.CollectionStore().Give(ctx, collectionstore.GiveParams{UserID: uint64(to), CharacterID: charID, UserID_2: uint64(from)})
	if err != nil {
