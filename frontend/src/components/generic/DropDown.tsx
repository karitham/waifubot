...
export function DropDownOptionWithImage(props: {
  ...
  return (
    <div class="flex flex-row items-center justify-between px-4 py-2 gap-4 hover:bg-surfaceC">
      <span>{props.label}</span>
      <Show when={props.image} fallback={<div />}>
        <img src={props.image} class="h-16 object-cover" />
      </Show>
    </div>
  );
}
...
export const DropDownSelect = <T extends readonly any[]>(props: {
  ...
  return (
    <DropDown
      ...
      icon={(open) => (
        <Show when={open} fallback={<span class="i-ph-caret-down" />}>
          <span class="i-ph-caret-up" />
        </Show>
      )}
    />
  );
